...
const MaintenanceSchedule = () => {
	...
	const events = schedules.map((s) => ({
		id: s.id,
		title: `${s.deviceId?.name || "Thiết bị"} - ${
			s.typeId?.type || "Bảo trì"
		}`,
		...
		try {
			...
			if (resolvedSchedule) {
				...
				const frequency = frequencies.find(
					(f) => f.id === resolvedSchedule.frequencyId.id
				);
			...
			const updatedSchedulesAfterResolve = schedules.filter(
				(s) => s.id !== id
			);
		...
		<div style={{ padding: "20px" }}>
			...
			{notifications.overdue.length > 0 && (
				<Alert variant="danger">
					...
					{notifications.overdue.length} thiết bị quá hạn bảo trì:
					<ul className="mt-2">
						{notifications.overdue.map((s) => (
							<li key={s.id}>
								<strong>{s.deviceId?.name}</strong> – ngày bảo
								trì:{" "}
			...
			{notifications.upcoming.length > 0 && (
				<Alert variant="warning">
					<strong>⏰ Nhắc nhở:</strong> Có{" "}
					{notifications.upcoming.length} thiết bị sắp đến hạn bảo
					trì:
					<ul className="mt-2">
