			...
			<Container fluid>
				...
				<Navbar.Collapse id="navbarScroll">
					<Nav className="me-auto my-2 my-lg-0" style={{ maxHeight: '100px' }} navbarScroll>
						<Link to="/devices" className="nav-link">{t("Home")}</Link>
						...
						{user === null ? <>
							...
							<Link to="/login" className="nav-link text-danger">{t("login")}</Link>
						</> : <>
							<img src={user.avatar} alt="avatar" className="rounded-circle" width="30" height="30" />
							<Link to="/profile" className="nav-link text-success">{t("greeting", { username: user.username })}</Link>
